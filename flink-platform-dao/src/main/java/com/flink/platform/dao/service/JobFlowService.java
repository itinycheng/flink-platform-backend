...
public class JobFlowService extends ServiceImpl<JobFlowMapper, JobFlow> {
    ...
    public void deleteAllById(long flowId) {
        ...
        jobFlowRunService.remove(new QueryWrapper<JobFlowRun>().lambda().eq(JobFlowRun::getFlowId, flowId));
        remove(new QueryWrapper<JobFlow>().lambda().eq(JobFlow::getId, flowId));
    }
