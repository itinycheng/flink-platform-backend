...
public class SortedMapTest {
    ...
    public void test() {
        ...
        JobCommand dependentCommand = new DependentCommand(1, false);
        dependentCommand.setExpectedStopTime(now);
        map.put(1L, dependentCommand);

        dependentCommand = new DependentCommand(3, false);
        dependentCommand.setExpectedStopTime(now.plusSeconds(100));
        map.put(3L, dependentCommand);

        dependentCommand = new DependentCommand(4, false);
        map.put(4L, dependentCommand);

        dependentCommand = new DependentCommand(5, false);
        map.put(5L, dependentCommand);

        dependentCommand = new DependentCommand(2, false);
        dependentCommand.setExpectedStopTime(now.minusSeconds(100));
        map.put(2L, dependentCommand);

        dependentCommand = new DependentCommand(6, false);
        map.put(6L, dependentCommand);

        dependentCommand = new DependentCommand(7, false);
        dependentCommand.setExpectedStopTime(now);
        ...
        Assert.assertEquals(7, map.getFirst().getJobRunId());
