...
public enum Placeholder {
    JOB_RUN(
            "${jobRun",
            (JobRunInfo jobRun, String content) -> {
                    ...
                    (JobRunInfo jobRun, String content) -> {
                        ...
                        while (matcher.find()) {
                            ...
                            if (StringUtils.isNotBlank(namespace) && StringUtils.isNotBlank(key)) {
                                result.put(
                                        paramName, pluginService.getApolloConfig(namespace, key));
                            }
                        }
                        return result;
                    })),
                        ...
                        while (matcher.find()) {
                            ...
                            if (StringUtils.isBlank(filePath)) {
                                throw new RuntimeException(
                                        "Hdfs path not found, variable:" + variable);
                            }
    ...
    Placeholder(
            @Nonnull String wildcard,
            @Nonnull BiFunction<JobRunInfo, String, Map<String, Object>> provider) {
