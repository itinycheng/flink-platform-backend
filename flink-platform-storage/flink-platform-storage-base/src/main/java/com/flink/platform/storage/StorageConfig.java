...
public class StorageConfig {
    ...
    public String createStorageBasePath(StorageSystem storageSystem, StorageProperties properties)
            throws Exception {
        ...
        if (!storageSystem.exists(storageBasePath)) {
            if (storageSystem.mkdir(storageBasePath)) {
                ...
                throw new RuntimeException(
                        format("create storage base dir: %s failed.", storageBasePath));
            }
        } else {
    ...
    @Bean("clusterIdPath")
    public String createClusterId(StorageSystem storageSystem, StorageProperties properties)
            throws Exception {
        String storageBasePath = properties.getStorageBasePath();
        ...
        if (!storageSystem.exists(clusterIdFile)) {
            ...
            String tmpClusterIdFile =
                    String.join(FILE_SEPARATOR, storageBasePath, ".main_cluster_id" + clusterId);
