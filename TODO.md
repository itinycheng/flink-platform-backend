...
## Apollo Config Loading — Decouple from Maven `${env}` Build-Time Substitution

`flink-platform-plugin-apollo` currently relies on a Maven antrun task that copies
`apollo-${env}.properties` into `META-INF/app.properties` at build time, with `${env}`
coming from the `<profiles>` section in the root `pom.xml` (`dev` / `test` / `prod`).
The same `${env}` placeholder also drives `flink-platform-ui`'s `npm run build:${env}`.

### Problem

- The active environment is **baked into the jar at build time** — same jar cannot be
  reused across dev / staging / prod.
- Maven build profile must match the deployment target; mismatch produces a jar that
  silently uses the wrong Apollo meta server.
- `pom.xml <profiles>` block exists solely to feed `${env}` to two consumers (Apollo +
  UI) — it is dead weight from the platform code's perspective.

### What's Wanted

- Build a single jar that can target any environment.
- Pick Apollo config at startup based on Spring profile (`SPRING_PROFILES_ACTIVE`) or a
  dedicated parameter, mirroring how `application-${profile}.yml` and
  `storage-${profile}.yml` already work.
- Remove the antrun copy step + the Maven `<profiles>` block once Apollo and UI both
  consume a runtime-driven mechanism.

### Sketch

- Bundle all `apollo-<env>.properties` into the jar as classpath resources.
- At Spring boot startup, read the active profile and load the matching properties into
  `System.properties` before Apollo's `ConfigService` initializes (Apollo client honors
  `apollo.meta` etc. as system properties).
- For UI, bake all environments at build time (single static bundle that detects env at
  runtime), or move env-specific config to runtime injection.

### Out of Scope (for now)

- Acted on when the build/deploy story is consolidated. Single-env teams can keep the
  current build-time substitution.
